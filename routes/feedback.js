...
router.post('/', auth, authorize('patient'), [
  ...
  try {
    ...
    // Send notification to practitioner about new feedback
    try {
      const practitionerUser = await User.findById(appointment.practitioner.userId);
      if (practitionerUser) {
        const subject = `New Feedback Received for ${therapyType} session`;
        const message = `Dear Dr. ${practitionerUser.firstName},\n\nYou have received new feedback for a ${therapyType} session from ${isAnonymous ? 'an anonymous patient' : patient.userId.firstName + ' ' + patient.userId.lastName}.\n\nOverall Rating: ${rating.overall}/5\nComments: ${comments || 'N/A'}\n\nPlease review the feedback in your dashboard.\n\nBest regards,\nPanchakarmaPro Team`;
        await notificationService.sendEmail(practitionerUser.email, subject, message);
        // Optionally send in-app notification
        const newNotification = new Notification({
          recipient: practitionerUser._id,
          type: 'feedback_received',
          title: subject,
          message: `New feedback received for ${therapyType} session. Overall rating: ${rating.overall}/5.`,
          priority: 'medium',
          channels: [{ type: 'in-app', sent: true, sentAt: new Date(), deliveryStatus: 'delivered' }],
          data: { feedbackId: feedback._id, appointmentId: appointmentId }
        });
        await newNotification.save();
      }
    } catch (error) {
      console.error('Failed to send new feedback notification to practitioner:', error);
    }
