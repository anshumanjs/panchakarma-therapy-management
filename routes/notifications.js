...
router.post('/send', auth, [
  ...
  try {
    ...
    // Implement actual sending logic for different channels
    for (const channel of notification.channels) {
      let sendResult;
      const recipientUser = await User.findById(recipientId); // Fetch recipient user for email/phone
      if (!recipientUser) {
        console.error(`Recipient user ${recipientId} not found for notification.`);
        channel.deliveryStatus = 'failed';
        channel.errorMessage = 'Recipient user not found.';
        continue;
      }

      if (channel.type === 'email' && recipientUser.email) {
        sendResult = await notificationService.sendEmail(recipientUser.email, title, message);
      } else if (channel.type === 'sms' && recipientUser.phone) {
        sendResult = await notificationService.sendSMS(recipientUser.phone, message);
      } else if (channel.type === 'in-app') {
        // In-app notifications are already handled by creating the Notification document
        sendResult = { success: true, messageId: 'in-app' };
      } else {
        sendResult = { success: false, error: `Unsupported channel or missing recipient contact: ${channel.type}` };
      }

      channel.sent = sendResult.success;
      channel.sentAt = new Date();
      channel.deliveryStatus = sendResult.success ? 'delivered' : 'failed';
      if (sendResult.error) {
        channel.errorMessage = sendResult.error;
      }
    }

    await notification.save(); // Save notification with updated channel statuses
...
router.post('/bulk-send', auth, [
  ...
  try {
    ...
    for (const notification of createdNotifications) {
      const recipientUser = await User.findById(notification.recipient); // Fetch recipient user for email/phone
      if (!recipientUser) {
        console.error(`Recipient user ${notification.recipient} not found for bulk notification.`);
      ...
      for (const channel of notification.channels) {
        let sendResult;
        if (channel.type === 'email' && recipientUser.email) {
          sendResult = await notificationService.sendEmail(recipientUser.email, notification.title, notification.message);
        } else if (channel.type === 'sms' && recipientUser.phone) {
          sendResult = await notificationService.sendSMS(recipientUser.phone, notification.message);
        } else if (channel.type === 'in-app') {
          sendResult = { success: true, messageId: 'in-app' };
        } else {
          sendResult = { success: false, error: `Unsupported channel or missing recipient contact: ${channel.type}` };
